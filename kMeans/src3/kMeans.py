...
#        print means 
        if length == 1 or length == 2 or length == 3:
            if length == 2:
                pointsX, pointsY = list(zip(*points))
                meansX, meansY = list(zip(*means))
                newmeansX, newmeansY = list(zip(*newmeans))
            if length < 3:
...
def plotSets(quantizedSets,pausebuffer):
    ...
    for cluster in quantizedSets:
        ...
        pointsX = [x[0] for x in list(cluster.keys())]
        pointsY = [y[1] for y in list(cluster.keys())]
        pointsZ = [z[2] for z in list(cluster.keys())]
...
def checkErrors(quantizedSets):
    ...
    for i in range(n):
        ...
        flag = 1
        for point in list(cluster.keys()):
